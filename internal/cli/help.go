...
func showForEachHelp() {
	...
	fmt.Println("  " + cyan("{{ branch | hash_port }}") + " " + dim("Deterministic port 10000-19999 from branch"))
	fmt.Println("  " + cyan("{{ branch | sanitize_db }}") + " " + dim("Branch as a safe database identifier"))
...
func showStepHelp() {
	...
	fmt.Println("    " + yellow("--llm") + "          " + dim("Generate message using configured LLM"))
	fmt.Println()
	fmt.Println("  " + cyan("gren step eval") + " <template>")
	fmt.Println("    Expand template variables for the current worktree and print the result.")
	fmt.Println("    Useful in scripts to derive per-worktree ports and database names.")
	fmt.Println()
	fmt.Println("    " + cyan("{{ branch | hash_port }}") + "   " + dim("Deterministic port 10000-19999 from branch"))
	fmt.Println("    " + cyan("{{ branch | sanitize_db }}") + " " + dim("Branch as a safe database identifier"))
	fmt.Println("    " + dim("(see 'gren for-each --help' for the full variable list)"))
	fmt.Println()
	fmt.Println(bold("EXAMPLES"))
	...
	fmt.Println("  $ PORT=$(gren step eval '{{ branch | hash_port }}') npm run dev")
	fmt.Println()
}
...
func showHooksHelp() {
	...
	fmt.Println()
	fmt.Println("  Inline commands expand template variables (see " + cyan("gren for-each --help") + "):")
	fmt.Println()
	fmt.Println("    " + yellow("post-create") + " = " + green("\"docker compose up -d --scale db=1 -p {{ branch | sanitize_db }}\""))
	fmt.Println("    " + yellow("post-create") + " = " + green("\"echo PORT={{ branch | hash_port }} >> .env\""))
	fmt.Println()
	...
	fmt.Println(bold("RUNNING HOOKS IN A TERMINAL"))
	fmt.Println()
	fmt.Println("  External tools (e.g. a terminal-multiplexer plugin) can run a repo's")
	fmt.Println("  configured hooks in a real terminal pane with:")
	fmt.Println()
	fmt.Println("    " + cyan("gren hook-run --type post-create --interactive") + "  " + dim("(or --tty)"))
	fmt.Println()
	fmt.Println("  " + dim("--interactive forces every hook to inherit the terminal (so setup that"))
	fmt.Println("  " + dim("needs a TTY — 1Password op, make seed — works) and prompts for approval"))
	fmt.Println("  " + dim("(persisted per project). This runs the full hook config: inline, script,"))
	fmt.Println("  " + dim("named, branch-filtered, and user-level hooks."))
	fmt.Println()
